...
export default function VibeMap({ memories, onDateClick }: VibeMapProps) {
    ...
    const getColor = (intensity: number, count: number) => {
        if (count === 0) return 'bg-sage-100/50 dark:bg-dark-400/50';
        if (intensity <= 0.25) return 'bg-sage-200 dark:bg-sage-800';
        if (intensity <= 0.5) return 'bg-sage-300 dark:bg-sage-700';
        if (intensity <= 0.75) return 'bg-sage-400 dark:bg-sage-600';
        return 'bg-sage-500';
    };
    ...
    return (
        <div className="w-full bg-white/50 dark:bg-dark-200/50 backdrop-blur-sm rounded-2xl p-5 shadow-soft border border-sage-100/30 dark:border-dark-400/30 transition-colors duration-300">
            ...
            <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-1 bg-cream-100 dark:bg-dark-400 rounded-full px-1 py-0.5">
                        <button
                            onClick={() => navigateYear(-1)}
                            className="w-6 h-6 rounded-full flex items-center justify-center text-slate-400 hover:text-sage-600 dark:hover:text-sage-400 hover:bg-sage-100 dark:hover:bg-dark-500 transition-colors cursor-pointer"
                        >
                            <FiChevronLeft size={12} />
                        </button>
                        <span className="text-xs font-semibold text-slate-600 dark:text-slate-300 min-w-[36px] text-center">
                            {selectedYear}
                        </span>
                        <button
                            onClick={() => navigateYear(1)}
                            ...
                            className={`w-6 h-6 rounded-full flex items-center justify-center transition-colors cursor-pointer ${selectedYear >= getYear(new Date())
                                ? 'text-slate-200 dark:text-dark-600 cursor-not-allowed'
                                : 'text-slate-400 hover:text-sage-600 dark:hover:text-sage-400 hover:bg-sage-100 dark:hover:bg-dark-500'
                                }`}
                        >
                    ...
                    {availableYears.map((year) => (
                        <button
                            ...
                            className={`px-2.5 py-1 rounded-full text-[10px] font-medium transition-all cursor-pointer ${year === selectedYear
                                ...
                                : 'bg-sage-50 dark:bg-dark-400 text-slate-500 dark:text-slate-400 hover:bg-sage-100 dark:hover:bg-dark-500 hover:text-sage-700 dark:hover:text-sage-400'
                                }`}
                        >
                            {year}
                        </button>
                    ...
                    <div className="flex gap-0.5">
                        ...
                        <div className="flex flex-col justify-around pr-2 shrink-0" style={{ height: 7 * (cellSize + gap) - gap }}>
                            {['', 'Mon', '', 'Wed', '', 'Fri', ''].map((label, i) => (
                                <span key={i} className="text-[9px] text-slate-400 dark:text-slate-500 leading-none h-[14px] flex items-center">
                                    {label}
                                </span>
                            ))}
                        </div>
            ...
            <div className="flex items-center justify-end gap-1.5 mt-3">
                ...
                {[
                    'bg-sage-100/50 dark:bg-dark-400/50',
